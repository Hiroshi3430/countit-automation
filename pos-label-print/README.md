...
- Print labels ファイルは**作成しません**。商品行の追加・Print labels の Save は行いません
- Stock Count 編集画面を安全に離脱するため、Stock Count の Save は実行します
- 在庫確定・在庫反映・inventory processing・+Process 相当の操作は dry-run でも絶対に行いません
...
- Current stock = `-1` → 対象
- Current stock = `1` → 対象外
- Current stock が空・非数値（productCode あり） → **エラーで停止**（対象判定ができないため）
- productCode がない空行 → スキップ（`skippedItems` にログ記録）
...
- `skippedItems`: スキップした行（productCode が空の行・重複 productCode の 2 件目以降）
