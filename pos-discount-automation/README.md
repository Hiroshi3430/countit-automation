...
このツールは、TSV/CSV入力ファイルをもとに CountIT の商品割引を登録・終了・置き換えするための自動化ツールです。
...
```bash
node src/main.js --mode dry-run --input input/discount_input.tsv
```
...
標準の discount 入力形式は、`examples/discounts.tsv` と同じタブ区切りTSV形式です。
...
- `input/discount_input.tsv` は実運用ローカルファイルです。
- `input/discount_input.tsv` はGit管理しません。
- `examples/discounts.tsv` はサンプルとしてGit管理します。

初回はサンプルをコピーして使います。

```bash
cp examples/discounts.tsv input/discount_input.tsv
```
...
`result-log.csv` は実行ログのCSVであり、discount入力ファイルとは別物です。
...
以下はCountITにアクセスしません。`pos-discount-automation/` ディレクトリ内から実行するか、リポジトリルートからサブシェル形式で実行してください。

```bash
# pos-discount-automation/ 内から実行
node --check src/*.js
npm test
